...
def calc_mags(fname,incl,mode,rzone):
    flux = np.genfromtxt(fname)
    johnson = np.genfromtxt("scaling/johnson.dat",skip_header=2)
    ...
    d = 1
    lum = np.trapz(flux[:,1],flux[:,0])*(4*np.pi*(d*pctocm)**2)/(lsun)    
    
    myfile = open("magnitudes_MODE_"+str(mode), "a")
    myfile.write('%5.2f %3i %9.7e %7.5e %7.5e %7.5e %7.5e %7.5e\n' % (incl[0],rzone,lum,u,b,v,r,i))
    myfile.close()
